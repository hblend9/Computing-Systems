...
void *mm_malloc(size_t size) {
    ...
    for (block_t *curr = mm_heap_first; mm_heap_last != NULL && curr <= mm_heap_last;
        ...
        if (!is_allocated(curr)) {
            for (block_t *next_block = (void *) curr + get_size(curr);
                 mm_heap_last != NULL && next_block <= mm_heap_last;
                 next_block = (void *) next_block + get_size(next_block)) {
                if (!is_allocated(next_block)) {
                    size_t block_size = get_size(next_block) + get_size(curr);
                    set_header(curr, block_size, false);
                }
                else {
                    break;
                }
    ...
    // Splitting:
    if (block != NULL) {
        // Change size of block to size of what we need
        if (new_size > size) {
            // Put rest of block in a new block
            set_header(block, size, true);
            block_t *new_block = (void *) block + size;
            set_header(new_block, new_size - size, false);
...
void *mm_realloc(void *old_ptr, size_t size) {
    ...
    if (new_size < block_size) {
        memcpy(new_ptr, old_ptr,
               new_size - sizeof(size_t)); // don't want to copy over the header
    }
    else {
        memcpy(new_ptr, old_ptr, block_size - sizeof(size_t));
    }
    mm_free(old_ptr);
    return new_ptr;
}

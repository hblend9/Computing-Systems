...
void process_setup(pid_t pid, const char* program_name) {
    ...
    memory_foreach(ptable[pid].pagetable, PROC_START_ADDR, map_to_kernel_space);  
    map_to_nobody(ptable[pid].pagetable, (uintptr_t)NULL);
    map_to_user_space(ptable[pid].pagetable, CONSOLE_ADDR);
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        for (uintptr_t a = round_down(seg.va(), PAGESIZE);
            ...
            //map_to_user_space(ptable[pid].pagetable, a);
            uintptr_t p_addr = (uintptr_t) kalloc(PAGESIZE);
            memory_map(ptable[pid].pagetable, a, p_addr, PTE_PWU);
        }
    }

    // initialize data in loadable segments
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        void *mem = memory_virtual_to_physical(ptable[pid].pagetable, seg.va());
        memset(mem, 0, seg.size());
        memcpy(mem, seg.data(), seg.data_size());
    }
    ...
    uintptr_t p_addr = (uintptr_t) kalloc(PAGESIZE);
    memory_map(ptable[pid].pagetable, stack_addr, p_addr, PTE_PWU);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    uintptr_t p_addr = (uintptr_t) kalloc(PAGESIZE);
    if ((void *) p_addr == nullptr) {
        return -1;
    }
    memory_map(current->pagetable, addr, p_addr, PTE_PWU);
    void *mem = memory_virtual_to_physical(current->pagetable, addr);
